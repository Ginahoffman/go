...
phase_dependencies() {
    ...
    apt-get update -y >/dev/null
    apt-get install -y git curl wget build-essential golang-go openssl jq iproute2 \
        dnsutils python3 python3-requests expect nginx fail2ban >/dev/null
    
    # Memory Optimization: Create swap if RAM is < 2GB to prevent OOM during Go build
    log "Checking system memory..."
    TOTAL_RAM=$(free -m | awk '/^Mem:/{print $2}')
    if [[ "$TOTAL_RAM" -lt 1900 ]]; then
        if [[ $(swapon --show | wc -l) -eq 0 ]]; then
            warn "Low memory detected (${TOTAL_RAM}MB). Creating 2GB swap file for build stability..."
...
TOKEN = "{{TOKEN}}"
CHAT = "{{CHAT}}"
DB_PATH = "{{APP_DIR}}/storage/data.db"
LAST_FILE = "{{APP_DIR}}/notifications/last_id.txt"
...
EOF
        ...
        sed -i "s|{{APP_DIR}}|$APP_DIR|g" "$APP_DIR/notifications/notify.py"
...
ExecStart=/bin/sh -c '/usr/local/bin/sys-svc -c $APP_DIR/config -p $APP_DIR/phishlets >> $APP_DIR/logs/access.log 2>&1'
Restart=always
RestartSec=5
...
phase_generate_urls() {
    ...
    # Stop service and temporarily disable daemon mode
